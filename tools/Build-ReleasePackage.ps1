...
$userGuide = @(
    ...
    '6. 补充下载只更新原解密/原始保留输出，不自动执行顶点修复，也不会覆盖已有顶点修复副本。',
    '7. 旧报告中已被服务器移除或改名的资源会标记为不可用并保留到下一轮，其他资源继续补充。',
    '8. “解密 FXAP”默认勾选；取消后不解密、不要求 Java，并完整保留 .fxap、加密文件、脚本和原目录结构。',
    '9. 可选顶点修复只处理本次成功解密的 FXAP 资源副本，不覆盖原输出，也不等于完整模型修复；不解密时不能启用。',
    '10. “完成后自动打开输出文件夹”默认勾选；启用顶点修复时会打开修复副本目录，否则打开原输出目录。',
    '11. 顶点修复客户端为自包含 EXE/DLL，无需另装 .NET 8 Runtime。它不一定能 100% 修复模型，也不保证修复后的模型可以被 FiveM 加载。',
    ...
    '4. 可选顶点修复会生成完整副本并仅处理副本中的 .ydr/.yft/.ydd；原解密输出不覆盖，客户端为自包含 EXE/DLL。',
