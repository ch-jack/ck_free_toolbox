...
$userGuide = @(
    ...
    '3. 功能含服务器 Dump、解密 FXAP；可选顶点修复只处理本次成功解密的 FXAP 资源副本，不覆盖原输出，也不等于完整模型修复。',
    '4. “完成后自动打开输出文件夹”默认勾选；启用顶点修复时会打开修复副本目录，否则打开原解密目录。',
    '5. 顶点修复需要 .NET 8 Runtime。它不一定能 100% 修复模型，也不保证修复后的模型可以被 FiveM 加载。',
    '6. 任务结束后可打开输出目录，并查看本次 Markdown/JSON 报告。',
    '',
    'FXAP 文件夹解密：',
