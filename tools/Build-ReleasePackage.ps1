...
$userGuide = @(
    ...
    '4. “扫描移除后门”和“秒杀小哈”已内置；其他页面显示“组件缺失”时，可点击“安装组件”下载并校验最新稳定 GitHub Release。',
    '5. 如果 Blender 显示未安装或版本过低，先安装 Blender 4.2+（推荐 5.1），再点击选择并指定 blender.exe。',
    '6. NUI/RPF/载具武器声浪/扫描移除后门页面缺少 Python 时，点击“官网”自行安装 Python 3.7+；“秒杀小哈”优先使用独立 EXE，仅旧组件回退入口需要 Python。',
    ...
    '3. 资源下载遇到可恢复的 403、404、429、5xx、连接中断或超时时会有限重试；最终失败文件会记录在日志和 Markdown/JSON 报告。',
    '4. 点击“补充失败下载”并选择上一次报告，只重新下载失败文件并合并回原输出；仍失败时可使用新报告继续补充。',
    '5. 解密模式补充时会额外下载必要的 RPF/.fxap 前置文件，但不会重复下载其他成功文件。',
    '6. 补充下载只更新原解密/原始保留输出，不自动执行模型修复，也不会覆盖已有模型修复副本。',
    '7. 旧报告中已被服务器移除或改名的资源会标记为不可用并保留到下一轮，其他资源继续补充。',
    '8. “解密 FXAP”默认勾选；取消后不解密、不要求 Java，并完整保留 .fxap、加密文件、脚本和原目录结构。',
    '9. 可选模型修复只处理本次成功解密的 FXAP 资源副本，不覆盖原输出；不解密时不能启用。',
    '10. “完成后自动打开输出文件夹”默认勾选；启用模型修复时会打开修复副本目录，否则打开原输出目录。',
    '11. 启用模型修复需要系统安装 .NET 8 Runtime；页面会自动检测并提供官网。',
    '12. 任务结束后可打开输出目录，并查看本次 Markdown/JSON 报告。',
    '',
    'FXAP 文件夹解密：',
    '1. 选择直接包含 .fxap 的 resource，或包含多个 resource 的父目录。',
    '2. CFX server key 可留空；未提供 key 时优先查询组件内 grants.json，缺失或字段不兼容时才查询 Keymaster grants API。',
    '3. grants.json 和接口鉴权只由 fxap_only 处理；工具箱不读取、保存或传递 Bearer Token，客户端派生仍由 Cloudflare 完成。',
    '4. 可选模型修复会生成完整副本并仅处理副本中的 .ydr/.yft/.ydd；原解密输出不覆盖，并需要系统安装 .NET 8 Runtime。',
    '5. “完成后自动打开文件夹”默认勾选；有修复副本时打开副本，否则打开原解密目录。',
    '6. Node.js 18+ 需要外部安装；Java 可选，也可以在页面选择外部 Java 目录。',
    '7. 输出自动写到相邻的 <目录名>_decrypted；未安装 Java 时 Lua 保留为 .luac。',
    '8. 任务结束后可打开本次 Markdown/JSON 报告，也可从“报告历史”查看以往记录。',
    '',
    'RPF 转 FiveM：',
    ...
    '3. 确认载具、武器、声浪、混合与疑似项目后，点击“复制完整资源文件夹”。',
    ...
    '',
    '发布包不包含 Blender、Python、Node.js、Java、.NET 8 Runtime 或 FFmpeg。Blender 需要 4.2+（推荐 5.1），Python 需要 3.7+，图片压缩需要 FFmpeg/ffprobe，FXAP 页面需要 Node.js 18+；Java 8+ 仅用于 Lua 反编译，.NET 8 Runtime 仅在启用模型修复时需要。',
    'Python 缺失时 NUI/RPF/载具武器声浪/扫描移除后门页面会打开 Python 官网，安装后可选择安装目录中的 python.exe；“秒杀小哈”独立 EXE 不依赖 Python。',
    '正式发布包内置“扫描移除后门”和“秒杀小哈”；模型、NUI、RPF、载具武器声浪、服务器 Dump 和 FXAP 组件按需安装。',
...
$requiredPackageFiles = @(
    ...
    (Join-Path $packagePath 'app\pages\ModelRepairPage.ps1'),
