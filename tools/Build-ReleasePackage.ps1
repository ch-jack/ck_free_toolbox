...
$userGuide = @(
    ...
    '7. “解密 FXAP”默认勾选；取消后不解密、不要求 Java，并完整保留 .fxap、加密文件、脚本和原目录结构。',
    '8. 可选顶点修复只处理本次成功解密的 FXAP 资源副本，不覆盖原输出，也不等于完整模型修复；不解密时不能启用。',
    '9. “完成后自动打开输出文件夹”默认勾选；启用顶点修复时会打开修复副本目录，否则打开原输出目录。',
    '10. 顶点修复需要 .NET 8 Runtime。它不一定能 100% 修复模型，也不保证修复后的模型可以被 FiveM 加载。',
    '11. 任务结束后可打开输出目录，并查看本次 Markdown/JSON 报告。',
    '',
    'FXAP 文件夹解密：',
